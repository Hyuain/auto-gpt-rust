...
pub fn print_project_scope(_project_description: &str) {
    ...
    ///     {
    ///         "is_crud_required": bool, // true is site needs CRUD
    ///         "is_user_login_and_logout": bool, // true if site needs users to bee able to login and log out
    ...
    ///         "is external_urls_required": false
    ///     }
    println!(OUTPUT)
}
...
pub fn print_site_urls(_project_description: &str) {
    ...
    /// Important: Only selects urls endpoint(s) which do not require any API Keys at all
